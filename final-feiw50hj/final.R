...
name <- "Pranay Singal"
github.name <- "psingal001"
email.address <- "psingal@berkeley.edu"
...
# y <- <your code here>
y <- list(seq(1, 1000, by = 1), seq(2, 1000, by = 2), seq(3, 1000, by = 3), 
          seq(4, 1000, by = 4), seq(5, 1000, by = 5), seq(6, 1000, by = 6),
          seq(7, 1000, by = 7), seq(8, 1000, by = 8), seq(9, 1000, by = 9),
          seq(10, 1000, by = 10))
...
#cw4 <- <your code here>
cw4 <- ChickWeight[sample(nrow(ChickWeight), size = 200),]
...
regres_line <- lm(mtcars$hp ~ mtcars$qsec)
abline(coef(regres_line)[[1]], coef(regres_line)[[2]])
...
plot(mtcars$wt, mtcars$disp, ylab = "Engine Displacement (cubic inches)", 
     xlab = "Weight of Car (Lb/1000)", col = ifelse(mtcars$am == 0, "Black", "Red"))
...
# Assumption: initially stats life expectancy vs. fertillity, so assume it to be x vs. y.
#      Also because need a vertical, not horizontal line at fertility rate of 2.1
...
par(mfrow = c(1, 2))
plot(WorldBank$life.expectancy[WorldBank$year == 1960] ~ WorldBank$fertility.rate[WorldBank$year == 1960],
     pch = ".", col = WorldBank$region, ylab = "Life Expectancy", xlab = "Fertility Rate")
plot(WorldBank$life.expectancy[WorldBank$year == 2013] ~ WorldBank$fertility.rate[WorldBank$year == 2013],
     pch = ".", col = WorldBank$region, ylab = "Life Expectancy", xlab = "Fertility Rate")
...
par(mfrow = c(1, 1))
hist(WorldBank$GDP.per.capita.Current.USD[WorldBank$lending == "IDA"],
     xlab="GDP Per Capita", main = "GDP for Countries with Lending Rating IDA",
     breaks = (max(WorldBank$GDP.per.capita.Current.USD[WorldBank$lending == "IDA"], na.rm = TRUE) - 
       min(WorldBank$GDP.per.capita.Current.USD[WorldBank$lending == "IDA"], na.rm = TRUE))/250)
...
prop.rain  <- sapply(rain, function(x) sum(x > 0))/sapply(rain, length)
...
# Assumption: In the output it says to return a vector and an optional plot. So in my return function
#   I returned a list with the first entry being the vector and the second being the histogram

GenNorm <- function(mean = 0, sd = 1, n = 100, plot.hist = TRUE){
  n_rand <- rnorm(n = n, mean = mean, sd = sd)
  if (plot.hist){
    hist_n.rand <- hist(n_rand)
  }
  return (list(n_rand, hist_n.rand))
}
...
#     we standardize by subtracting the mean of the column/row and 
#        dividing by the sd of the column/row


standardizeVar <- function(m, cols=TRUE){
  m_stand <- matrix(nrow = nrow(m), ncol = ncol(m))
  if (cols){
    for (i in 1:ncol(m)){
      mean_xi <- mean(x[,i])
      sd_xi <- sd(x[,i])
      m_stand[,i] <- (x[,i]-mean_xi)/sd_xi
    }
  } else{
    for (i in 1:nrow(m)){
      mean_xi <- mean(x[i,])
...
PermDiff <- function(cases, controls, k=5000){
   helper_func <- function(cases, controls){
     all_obs <- c(cases, controls)
     new_cases <- sort(sample(all_obs, size = length(cases)))
     new_controls <- all_obs[!all_obs %in% new_cases]
     return(mean(new_cases) - mean(new_controls))
   }
  replicate(k, helper_func(cases, controls))
}
...
NumJackpot <- function(k, B){
  game <- function(k){
    jackpot_winner <- 0
    numbers <- c(1:19)
    winning_numbers <- sort(sample(numbers, size = 3))
    for (i in 1:k){
      player_numbers <- sort(sample(numbers, size = 3))
      if (all(player_numbers == winning_numbers)){
        jackpot_winner <- jackpot_winner + 1
      }
    }
    return(jackpot_winner)
  }
  
  n.jackpot <- replicate(B, game(k))
  return (n.jackpot)
}

# For B = 50 and each value of k = 10000, 50000, 100000, 500000
...
phrases <- c("stone", "steel", "sled", "Star", "silly", "cat", "dog", "catcat", "why?", 
             "really!", "how much?", "atatat", "bar", "car")

# [2 pts]
# Create a vector [text1] that lists the elements in phrases that start
# with "st" (case doesn't matter)
#text1 <- <your code here>
text1 <- phrases[substr(phrases, 0, 2) == "st" | substr(phrases, 0, 2) == "St"]
...
text2 <- phrases[substr(phrases, nchar(phrases)-1, nchar(phrases)) == "ar"]
...
match1 <- sort(unique(c(grep("A.T", dna), grep("G.T", dna))))
