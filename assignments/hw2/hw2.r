...
plot(wr1500m[,"year"], wr1500m[,"times"], type = "p")
...
plot(wr1500m[,"new_year"], wr1500m[,"times"], type = "p")
...
plot(wr1500m[,"new_year"], wr1500m[,"times"], type = "p", xlim = c(1892,2014))
lines(c(1892, 2014), c(wr_1998,wr_1998))
...
lines(c(1892, 2014), c(wr_1998,wr_1998))
abline(v = wr_1944, col = "light gray", lwd = 2)
...
plot(wr1500m[,"new_year"], wr1500m[,"times"], type = "p", xlim = c(1892,2014), main = "Men's 1500m World Record Times", xlab = "Year", ylab = "Time in Seconds")
abline(v = wr_1944, col = "light gray", lwd = 2)
abline(v = wr1500m$new_year[wr1500m$times == wr_1998], col = "light gray", lwd = 2)
text(1930, 245, labels = wr1500m$athlete[wr1500m$year == 1944], cex = 5)
text(1980, 220, labels = wr1500m$athlete[wr1500m$year == 1998], cex = 5)
...
plot(SO2012Ctry$GDP, SO2012Ctry$population)
...
symbols(log(SO2012Ctry$pop),log(SO2012Ctry$GDP_per_person), circle = SO2012Ctry$Total)
...
symbols(log(SO2012Ctry$pop)[SO2012Ctry$Total>0],log(SO2012Ctry$GDP_per_person)[SO2012Ctry$Total>0], circle = SO2012Ctry$Total[SO2012Ctry$Total > 0])
points(log(SO2012Ctry$pop)[SO2012Ctry$Total>0], log(SO2012Ctry$GDP_per_person)[SO2012Ctry$Total>0], pch = ".")
...
top5 <- order(SO2012Ctry$Total, na.last = NA)
# your plotting code here, including a new call to text() 
symbols(log(SO2012Ctry$pop)[SO2012Ctry$Total>0],log(SO2012Ctry$GDP_per_person)[SO2012Ctry$Total>0], circle = SO2012Ctry$Total[SO2012Ctry$Total > 0], main = "Medals Won Relative to Population and GDP per Capita", xlab = "log of Population", ylab = "log of GDP per Capita")
points(log(SO2012Ctry$pop)[SO2012Ctry$Total>0], log(SO2012Ctry$GDP_per_person)[SO2012Ctry$Total>0], pch = ".")
...
world <- subset(SO2012Ctry, SO2012Ctry$Country %in% wonMedal)
symbols(world$longitude, world$latitude, circles = world$Total, 
        xlim = c(-180,180), ylim = c(-90,90), add=T, inches = .45)
