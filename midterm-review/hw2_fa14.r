...
load("C:/Users/Pranay/src/stat133/midterm-review/family.rda")
...
# Finally, use OW_limit and bmi to create the desired logical vector, called
...
load("C:/Users/Pranay/src/stat133/midterm-review/SFHousing.rda")
...
BerkArea <- BerkArea[(BerkArea$price <= quantile(BerkArea$price, p = .99, na.rm = TRUE)) & 
                       (BerkArea$bsqft <= quantile(BerkArea$bsqft, p = .99, na.rm = TRUE)),]
...
plot(BerkArea$year, BerkArea$pricePsqft,
     data = BerkArea, 
     main = "Housing prices in the Berkeley Area",
     xlab = "Year house was Built",
     ylab = "Price per square foot",
     col = brCols, pch = 19, cex = 0.5)
legend("topright", legend = 1:5, fill = rCols)
